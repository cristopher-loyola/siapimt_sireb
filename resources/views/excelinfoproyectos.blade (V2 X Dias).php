...
use App\Models\Tarea;
	...
	//Obtiene los días hábiles de todo el proyecto
        // Incremento en 1 dia
        $diainc = 24*60*60;
       
        // Arreglo de dias habiles, inicializacion
            $dh=0;
       	    $midia= $fechainicio;
        
        // Se recorre desde la fecha de inicio a la fecha fin, incrementando en 1 dia
        while( $midia <= $fechafin) {
...
/* Inicio del Excel */
    ...
    /* Datos y construccion del Cronograma Inicio */
 	...
 	$diasferiados=array("2022-01-01","2022-02-07","2022-03-21","2022-04-14","2022-04-15","2022-05-01","2022-05-05","2022-09-16","2022-11-02","2022-11-21","2022-12-25");
    $tdiasp=0;
    $ar=9;
foreach ($tarea as $ta) {
    $finita=strtotime($ta->fecha_inicio);
    $ffinta=strtotime($ta->fecha_fin);
	$diasp= getDiasHabiles($finita,$ffinta,$diasferiados);  
    $tdiasp=$tdiasp+$diasp;
    $ar=$ar + 2;
  }
...
foreach ($tarea as $ta) {
	...
	/*Encuentra la fecha de inicio de la tarea dentro de la matriz de meses*/
	           ...
	           $zh=1+$difm;
               $colfin=69+$proyt->duracionm;
               $cf=convchr($colfin);
               $cf2=convchr($colfin+1);
               
	   //  $diashpm=getDHpm($finita,$ffinta,$diasferiados,$proyt->duracionm);

// Obtiene los días hábiles por mes de la tarea
         $durmeses=$proyt->duracionm;
        // Incremento en 1 dia
        $diainc = 24*60*60;
       $dh=0;
   for ($i=1;$i<=$durmeses;$i++){
              $diashpm[$i]=0; 
	} 
	
       $mi=date('m',$finita);
       $fciclo=$ffinta+$diainc;
    while(in_array(date('N', $fciclo), array(6,7)) or in_array(date('Y-m-d', $fciclo), $diasferiados))
 { $fciclo=$fciclo+$diainc;}
       
        // Se recorre desde la fecha de inicio a la fecha fin de la tarea, incrementando en 1 dia
        for ($midia = $finita,$m=1; $midia <= $fciclo; $midia = $midia+$diainc) {
                // Si el dia indicado, no es sabado o domingo es habil
                if (!in_array(date('N', $midia), array(6,7))) { // DOC: http://www.php.net/manual/es/function.date.php
                        // Si no es un dia feriado entonces es habil
                        if (!in_array(date('Y-m-d', $midia), $diasferiados)) {
                               if($mi==date('m',$midia) and $midia<=$ffinta){
                                    $dh++;}
                               else{
                               	  $mi=date('m',$midia);
                               	  $diashpm[$m]=$dh;
		...
		for($ci=1;$ci<=$ta->duracion;$ci++){
                     $c=$c + $diashpm[$ci]; /*Cuenta el total de días por tarea*/
                   }

		 for ($i=$colinicro, $cdm=1; $i < $fin; $i++, $cdm++, $zh++)
          {     	//calcula la variable $real desde el arreglo díashpm del archivo rutina_dias_lab.php - función getDHpm
			$realpd=$diashpm[$cdm] * $apd;
            $mrealpd[$cdm]=$diashpm[$cdm] * $apd;
		    $asccolini= convchr($i);
		    ...
		    $cronograma->getStyle($asccolini.$cont)->getFont()->getcolor()->setARGB('FF000088');
                    ...
                    $cronograma->setCellValue($asccolini.$cont,round($realpd,2).'%');
                    $cronograma->getStyle($asccolini.$cont)->getFill()
                                      ->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)
                                        ->getStartColor()->setARGB('FF000088');
                  
                    $matp[$zh]=$matp[$zh]+$realpd;
                    $realpd=0.0000;                   
		   }
         ...
         /*si lo realizado es mayor a lo programado */
		 $i=1;
         $dias=0;
         while($mrealpd[$i]<=$f and $dias<$c)
         {
             $asccolini= convchr($colinicro);
                 ...
                 $cronograma->setCellValue($asccolini.$comb, round($mrealpd[$i],2).'%');
            $matr[$zk]=$matr[$zk]+$mrealpd[$i];
            $dias=$dias+$diashpm[$i];
             $f=$f-$mrealpd[$i];
             $i++;    
             $zk++;
        ...
        if($p<100){
             $asccolini= convchr($colinicro);
                 $cronograma->getStyle($asccolini.$comb)->getFill()
                                        ...
                                        ->getStartColor()->setARGB('FF32CD32');
                 $cronograma->getStyle($asccolini.$cont)->getFont()->getcolor()->setARGB('FF32CD32');
                 $cronograma->setCellValue($asccolini.$comb, round($f,2).'%'); 
                 $matr[$zk]=$matr[$zk]+$f;
             }
        ...
        $cont;
        $treal=0;
        for($ci=1;$ci<=$proyt->duracionm;$ci++){
                     $treal=$treal + $matr[$ci]; /*Cuenta el total de lo realizado*/
                   }
        ...
        $cronograma->setCellValue('D'.$comb,round($treal).'%');
        ...
        /* Formato observaciones  Inicio */
            ...
            $documento->getActiveSheet()->getStyle('C'.$extra2.':'.$letra.$extra)
                    ->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER)
                    ->setVertical(\PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_CENTER)->setWrapText(true);
        /* Formato Observaciones fin */
        /* Formato Pie de página inicio */
        $pie = $comb+5;
