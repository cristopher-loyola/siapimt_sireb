...
<p>Buen día {{ $details['participante']->Nombre }}, se le notifica que {{ $details['servicio']->encargado }} lo ha excluido en el Sistema de Reportes Bimestrales, como participante de:</p>
<br>
<p>Evento: {{ $details['evento']}}</p>
<p>Fecha: {{ $details['servicio']->fechainicio }}</p>
<p>Titulo: {{ $details['servicio']->nombreservicio }}</p>
<br>
<br>
<p>Para cualquier aclaración, comunicarse con la persona que lo excluyó.</p>
<p>Por favor, no responda a este mensaje, ya que ha sido enviado de forma automática.</p>

<p style="color: #4c642c; font-size: 12px;">
    <img src="{{ asset('../img/Arbolito.png') }}" style="height: 12px;"> Antes de imprimir este mensaje, asegúrese de que es necesario.
