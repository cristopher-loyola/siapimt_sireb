...
/*
if (!$validLocale {
    echo 'No se encontro el paquete'.$locale."cambio a en_US".PHP_EOL;
}*/
...
/* Consultas para el rellenar el documento Inicio*/
    ...
    $obs = Observacion::where('idproyecto', $proyt->id)->get();
...
/* Inicio del Excel */
    ...
    /* Estilo para la primera hoja Inicio*/
        /* Estilos de las celdas Inicio*/
            ...
            /* Tipo de letra Global Incio */
                ...
                $documento->getActiveSheet()->getStyle('B2')->getFont()->setSize(10)->setBold(true)->setName('Arial');
            ...
            /* Borde de las columnas Inicio */
                ...
                $arraynume = array(7,8,9,10,11,12,13);
    ...
    /* Datos y construccion de la portada inicio */
        ...
        /* Fila 2 */
        $portada->setCellValue("B2", "Área de adscripción");
        ...
        $portada->setCellValue('B4', 'DATOS GENERALES');
         /* Fila 5 */
        $portada->setCellValue("A5", "Clave y nombre del proyecto:");
        $portada->mergeCells("B5:E5");
		if ($proyt->claven<10) {
                $portada->setCellValue("B5",
                $proyt->clavea.$proyt->clavet.'-0'.$proyt->claven.'/'.$proyt->clavey.' '.$proyt->nomproy);
            } else {
                $portada->setCellValue("B5",
                $proyt->clavea.$proyt->clavet.'-'.$proyt->claven.'/'.$proyt->clavey.' '.$proyt->nomproy);
            }
               /* Fila 6 */
        ...
        $portada->setCellValue("D13", "Modo de transporte");

        if ($proyt->otrotrans == 'N/A')
        {  $portada->setCellValue("E13", $tran->nombre_transporte); }
        else { $portada->setCellValue("E13", $proyt->otrotrans); }
    ...
    /* Datos y construccion del Cronograma Inicio */
        ...
        /* Fila 2 */
        $cronograma->setCellValue("B2", "Área de adscripción");
        ...
        $cronograma->setCellValue('B5', 'PROGRAMA DE ACTIVIDADES');
         /* Fila 6 */
        $cronograma->setCellValue("A6", "Clave y nombre del proyecto:");
 	$cronograma->mergeCells("B6:K6");
		if ($proyt->claven<10) {
            $cronograma->setCellValue("B6",
            $proyt->clavea.$proyt->clavet.'-0'.$proyt->claven.'/'.$proyt->clavey.' '.$proyt->nomproy);
            } else {
                $cronograma->setCellValue("B6",
                $proyt->clavea.$proyt->clavet.'-'.$proyt->claven.'/'.$proyt->clavey.' '.$proyt->nomproy);
        ...
        /* Inicio Genera la matriz para colocar la fecha en la parte superior del cronograma*/
            $lc=69;
            $im=1;
            ...
            while ($im <= $proyt->duracionm) {
                ...
                // inicio Cambia las fechas de ingles a español, elimina caracteres especidificos
                    $finiciop2 = strftime("%b-%y", $finiciop);
                    $finiciop3 = ucfirst($finiciop2);
                    $finiciop4 = str_replace('.', '', $finiciop3);
                    $cronograma->setCellValue($col."8", $finiciop4);
                //fin de cambios
                $mmes[$im]=date("M-y", $finiciop); /*crea una matriz con mes y año para después
                comparar y conocer la columna donde inicia cada tarea */
                $finiciop = strtotime("+1 month", $finiciop);
                $lc++;
                $im++;
            }
        /* Fin Genera la matriz para colocar la fecha en la parte superior del cronograma*/
	   ...
	   $totalr=0;
                ...
                // Code 98% nuevo
                    if ($proyt->clavet == 'I') {
                        if ($proyt->publicacion == 1){
                            ...
                            if (round($totalr) == 100) {
                                $cronograma->setCellValue(convchr($colini).$comb, '98%');
                            } else {
                                $cronograma->setCellValue(convchr($colini).$comb, round($totalr).'%');
                            }
                        }
        ...
        /* Suma de porcentajes Fin */
            /* Convierte la celda a porcentaje Inicio */
                ...
                $documento->getActiveSheet()->getStyle('D'.$cont)
                ->getNumberFormat()->setFormatCode(\PhpOffice\PhpSpreadsheet\Style\NumberFormat::FORMAT_PERCENTAGE);
               $documento->getActiveSheet()->getStyle('E1:'.$letra.$comb)
               ->getNumberFormat()->setFormatCode(\PhpOffice\PhpSpreadsheet\Style\NumberFormat::FORMAT_PERCENTAGE);
            ...
            /* Formato de las celdas Inicio */
               ...
               $documento->getActiveSheet()->getStyle('E'.$cont.':'.$letra.$comb)
                ...
                for ($i=9; $i<$cont; $i+=2) {
                    ...
                    ->getStyle('E'.$i.':'.$letra.$i)->getFont()->getcolor()->setARGB('FF000088');
                }

                for ($i=10; $i<$comb; $i+=2) {
                    $documento->getActiveSheet()
                    ->getStyle('E'.$i.':'.$letra.$i)->getFont()->getcolor()->setARGB('FF32CD32');
                }
            ...
            // Observaciones
                ...
                foreach ($obs as $obs) {
                    if ($obs->revisado == 1) {
                        if ($proyt->estado == 4 && $obs->tipo == 1) {
                            $cronograma->setCellValue('C'.$extra2, $obs->obs);
                        } elseif ($proyt->estado == 5 && $obs->tipo == 2) {
                            $cronograma->setCellValue('C'.$extra2, $obs->obs);
                        } else {
